...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const visualizationContainer1 = document.getElementById(
    "visualization-container-1"
  );
  const visualizationContainer2 = document.getElementById(
  ...
  const ALGORITHMS = {
    bubble: {
      ...
      explanation:
        "Bubble Sort compares adjacent elements and swaps them if they are in the wrong order. This process is repeated until the entire list is sorted. It's called bubble sort because smaller elements 'bubble' to the top.",
    },
    selection: {
      name: "Selection Sort",
      description:
        "In-place comparison sort that divides input into sorted and unsorted regions",
      complexity: {
        best: "O(n²)",
        average: "O(n²)",
        worst: "O(n²)",
      ...
      explanation:
        "Selection Sort works by finding the minimum element from the unsorted part and putting it at the beginning. It maintains two subarrays: sorted and unsorted.",
    },
    insertion: {
      name: "Insertion Sort",
      description:
        "Efficient algorithm for small datasets, builds final sorted array one item at a time",
      complexity: {
        best: "O(n)",
        average: "O(n²)",
        worst: "O(n²)",
      ...
      explanation:
        "Insertion Sort builds the sorted array one element at a time by repeatedly taking an element from the unsorted part and inserting it into the correct position in the sorted part.",
    },
    merge: {
      name: "Merge Sort",
      description:
        "Divide-and-conquer algorithm with guaranteed O(n log n) performance",
      complexity: {
        best: "O(n log n)",
        average: "O(n log n)",
        worst: "O(n log n)",
      ...
      explanation:
        "Merge Sort uses the divide-and-conquer approach. It divides the array into halves, sorts them recursively, and then merges the sorted halves back together.",
    },
    quick: {
      name: "Quick Sort",
      description:
        "Highly efficient divide-and-conquer algorithm with excellent average-case performance",
      complexity: {
        best: "O(n log n)",
        average: "O(n log n)",
        worst: "O(n²)",
      ...
      explanation:
        "Quick Sort picks a 'pivot' element and partitions the array around it, such that elements smaller than pivot come before it and greater elements come after. It then recursively sorts the subarrays.",
    },
    heap: {
      name: "Heap Sort",
      description:
        "Comparison-based algorithm using binary heap data structure",
      complexity: {
        best: "O(n log n)",
        average: "O(n log n)",
        worst: "O(n log n)",
      ...
      explanation:
        "Heap Sort uses a binary heap data structure. It first builds a max heap from the array, then repeatedly extracts the maximum element and places it at the end of the sorted portion.",
    },
  };
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  function selectOptimalAlgorithm(analysis, array) {
    ...
    if (hasLowVariance && size > 20) {
      return {
        ...
        confidence: 0.89,
        reason:
          "Guaranteed O(n log n) performance for arrays with few unique values",
    ...
    if (size <= 40 && !hasLowVariance) {
      return {
        ...
        confidence: 0.87,
        reason:
          "Excellent average-case performance for medium-sized random arrays",
  ...
  function displayRecommendation(recommendation, analysis) {
    ...
    if (timeEstimate)
      timeEstimate.textContent = `~${recommendation.estimatedTime}ms`;
  ...
  async function startComparisonSorting() {
    ...
    if (algo1 === algo2) {
      showMessage(
        "Please select different algorithms for comparison.",
        "warning"
      );
      return;
    }
    ...
    updateExplanation(
      `Comparing ${ALGORITHMS[algo1].name} vs ${ALGORITHMS[algo2].name}...`
    );
  ...
  function updateComparisonStats() {
    if (algo1Comparisons)
      algo1Comparisons.textContent = comparisonData.algo1.comparisons;
    if (algo1Swaps) algo1Swaps.textContent = comparisonData.algo1.swaps;
    if (algo2Comparisons)
      algo2Comparisons.textContent = comparisonData.algo2.comparisons;
  ...
  async function finalizeComparisonSorting(bars1, bars2, executionId) {
    ...
    updateExplanation(
      `✅ Comparison complete! 
            ${ALGORITHMS[algorithm1Select?.value || "bubble"].name}: ${
        comparisonData.algo1.comparisons
      } comparisons, ${comparisonData.algo1.swaps} swaps | 
            ${ALGORITHMS[algorithm2Select?.value || "selection"].name}: ${
        comparisonData.algo2.comparisons
      } comparisons, ${comparisonData.algo2.swaps} swaps`
    );
  }
  ...
  async function bubbleSort(array, bars, executionId, comparisonId = null) {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      for (let j = 0; j < n - i - 1; j++) {
        ...
        updateExplanation(
          `Comparing elements at positions ${j} and ${j + 1}: ${array[j]} vs ${
            array[j + 1]
          }`
        );
    ...
    for (let i = 1; i < n; i++) {
      ...
      if (bars && bars[j + 1]) {
        bars[j + 1].style.height = `${
          (key / Math.max(...initialArrayState)) * 300
        }px`;
  ...
  async function mergeSort(array, bars, executionId, comparisonId = null) {
    async function merge(left, right, start, mid, end) {
      ...
      while (i < left.length && j < right.length) {
        ...
        if (bars && bars[k]) {
          bars[k].style.height = `${
            (array[k] / Math.max(...initialArrayState)) * 300
          }px`;
    ...
    async function mergeSortRecursive(arr, start, end) {
      ...
      updateExplanation(
        `Dividing array from ${start} to ${end} at position ${mid}`
      );
      ...
      updateExplanation(
        `Merging subarrays from ${start} to ${mid} and ${mid + 1} to ${end}`
      );
  ...
  function clearVariables() {
    document
      .querySelectorAll('[id*="var-"][id*="-value"]')
      .forEach((element) => {
        element.textContent = "-";
      });
  }
  ...
  function createBarsForComparison() {
    ...
    if (arrayToSort.length === 0) {
      [visualizationContainer1, visualizationContainer2].forEach(
        (container) => {
          container.innerHTML = `
                    <div class="empty-state">
                        <i class="fas fa-chart-bar"></i>
  ...
  function disableControls() {
    ...
    if (analyzeSortBtn) {
      ...
      analyzeSortBtn.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> Sorting...';
    }
                ...
                <div class="algorithm-details">
                    ...
                    <div class="recommendation-details">
                        <div class="detail-item">
                            <strong>Confidence Level:</strong> ${Math.round(
                              recommendedAlgorithm.confidence * 100
                            )}%
                        </div>
                        <div class="detail-item">
                            <strong>Estimated Time:</strong> ~${
                              recommendedAlgorithm.estimatedTime
                            }ms
                        </div>
                        <div class="detail-item">
  ...
  function generateCode(algorithm, language) {
    ...
    if (!template) {
      showMessage(
        ...
        } in ${language} is not available yet.`,
        "warning"
      );
      return;
    }
    ...
    if (codeFilename)
      codeFilename.textContent = `${algorithm}_sort${fileExtensions[language]}`;
    if (generatedCode) {
